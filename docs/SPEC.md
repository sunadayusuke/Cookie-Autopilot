...
5. **層の順序**（成功したら終了。各層は非同期で、前の層の失敗を待って次へ）:
   ...
   d. **ヒューリスティック**（独自実装向け）:
      - **容器検出**（要素 1 つ分の判定は `evaluateContainer()`。`findContainers()` は全要素をこれに掛ける）: 判定は **①入口の足切り → ②ハード条件 → ③加点と閾値** の 3 層。採用したものが複数あれば最も内側（最小）を優先する。**ただしヒューリスティック（`heuristicContainers()`）が採用する容器は、面積の小さい順に 2 段で選ぶ**: ⒜ 対象モードで押せる候補（`scoreCandidates()`。**reject モードでは閉じる語（`close`）の候補を数えない**）がある最初の容器 → ⒝ 無ければ hide の見込みがある（`canHide()`）最初の容器 → ⒞ それも無ければ従来どおり先頭（accept モードでは ⒜ と ⒝ は一致する）。Borlabs の `_brlbs-content-blocker` のような埋め込みのプレースホルダ「This video is blocked because of your cookie settings.［Unblock video］」は Cookie を名乗るので容器として採用されるが、本物のバナー以下の面積だと先頭になり、reject が `no-reject` で終わって本物のバナーまで届かなかった。Cookiebot 公式のプレースホルダ `<div class="cookieconsent-optout-marketing">Please <a href="javascript:Cookiebot.renew()">accept marketing-cookies</a> to watch this video.</div>` は許可候補を持つので `canHide()` が真になり、先頭のままだと reject でプレースホルダが hide されて本物のバナーの "Reject all" が押されなかった。⒝だけの容器（拒否できず hide しかできない）より⒜の容器を先に使うのは、押してよい条件はどの容器もハード条件を通っていて同じで、hide はあくまで断れなかったときの代替だから（断れる容器があるならそちらを処理する。「内側の断片に許可だけ・外側の容器に拒否がある」構造でも外側の拒否を押す）。判定は fallback の設定（hide / leave）に依存させない（fallback=leave でも［Accept all］だけ＋設定導線のバナーは設定パネル層の対象なので、⒝で拾う）。**どの容器も⒜⒝に当たらなければ従来どおり先頭**を使い、設定パネル層・fallback の判断もこれまでと同じ容器で行う。**残存リスク**: 本物のバナーが無く Cookiebot 型のプレースホルダだけがあるページでは、従来どおり⒝でプレースホルダが hide されうる（既存の挙動で、今回は対象外）。また、**内包関係に無い 2 つの容器**（例: CMP のバナー［すべて許可］［Cookie 設定］と、別の Cookie 容器［拒否］）が同時にあると、⒜の拒否を押して処理を終えるので、⒝の容器は設定パネル層も hide も通らず画面に残る（修正前は⒝側を hide して⒜側が残っていた。どちらか一方が残る点は同じで、押すのは拒否なので同意は送られない）。⒜から閉じる語を除いているのは、テーマ付属の「使い続ければ同意」型の通知［OK］（実質的な同意）が、この理由で本物のバナーより先に押されないようにするため。飛ばした容器もリストからは消さず、hide の範囲の計算（`outermostContainer`）には残る。猶予中などに**採用が内包関係に無い別要素へ移ったら、前の容器の cloak を外す**（先に見えていたプレースホルダ＝動画の枠が opacity:0 のまま残らないように。BEM の外側⇄内側のような内包関係の切り替えでは、外すとバナーの一部がちらつくので外さない）。shadow DOM は deepQuery で横断（**root として渡された要素自身の `shadowRoot` にも入る**。`querySelectorAll('*')` は root 自身を含まないので、`#usercentrics-root` のような CMP のマウント点がそのまま容器になったときに中のボタンを 1 つも拾えなかった）。採用した容器は即 cloak
      ...
      - **② ハード条件**（1 つでも欠けたら容器にしない。この順に見る）:
        ...
        12. **もう選び終わったことを伝えるだけの案内でない**こと。可視テキストに **完了状態の文言** `/you.?ve chosen to reject|you have rejected|you.?ve rejected|already (rejected|accepted)|consent (choice )?saved|設定を保存しました|拒否しました|同意しました|受け付けました/i` があり、**決定ボタンが閉じる語（`CLOSE_EXACT`）か折りたたみ語（`COLLAPSE_EXACT` = `collapse` `collapse banner` `hide banner` `hide cookie banner` `折りたたむ` 等）しか無い**容器は、同意を求める画面ではないので**容器にしない**（＝何も押さず、hide もせず、`unhandled` にもしない）。theguardian.com の「You've chosen to reject third-party cookies while browsing our site.［Collapse banner］」を同意画面として報告してしまっていたため。文言だけでは「…に同意しましたか？［同意する］［同意しない］」のような同意要求にも当たるので、**必ず決定ボタンの側と組で見る**。`COLLAPSE_EXACT` はこの判定にしか使わず、閉じる語（自動で押す語）には入れない。`you.?ve rejected` は GOV.UK Frontend の選択後メッセージ「You've rejected additional cookies.［Hide cookie message］」向けの短縮形（正規化前のテキストに当てるので `.?` で `'` と `’` の両方に当たる）。**拒否側だけ**に足してあり、許可側の `you.?ve accepted` は入れない——「By using this site you've accepted our cookies.［OK］」型の通知まで完了状態と見なし、閉じる語で閉じられなくなるため
      ...
      - **正規化**（`normalize()`）: trim → 小文字化 → 全角英数記号（U+FF01–U+FF5E）を半角へ → 空白（全角スペース含む）と不可視の書式文字（`/\p{Default_Ignorable_Code_Point}/gu`。Unicode が「描画しない」と定めた文字の集合で、ソフトハイフン U+00AD・ゼロ幅スペース U+200B・ゼロ幅（非）接合子 U+200C / U+200D・単語結合子 U+2060・方向制御 U+200E / U+200F / U+202A–U+202E / U+2066–U+2069・結合書記素結合子 U+034F・異体字セレクタ U+FE0F・BOM U+FEFF などを含む。BOM 以外は `\s` に含まれない。1 文字ずつ列挙すると漏れた文字ですり抜けるのでプロパティで指定する）を除去 → 約物 `。！!.,、'’‘"“”?？…:：;；‐–—-()（）[]【】&＆+＋・»›«‹→` を除去。英語ボタンの揺れを語彙の正規表現で書き分けずに済ませるための処理で、"Don’t accept"→`dontaccept` / "opt-out"→`optout` / "Accept & Close"→`acceptclose` / "Why?"→`why` になる。閉じるボタンの `×` `✕` は語彙（CLOSE_EXACT）なので残す。不可視の書式文字を落とすのは、`Un&shy;block all` が `un〈U+00AD〉blockall` のまま拒否語の `(?<!un…)blockall$` の否定後読みをすり抜けて拒否の強一致になり、reject で押されていたため（見た目の文言と照合結果を一致させる。LRM / RLM は RTL 対応の CMS が翻訳文言の前後に自動で挟むことがあり、攻撃でなくてもふつうに混ざる）。`normalize()` は語彙・禁止語・hideTarget のテキスト比較・カスタムルールのすべての照合に効くが、どれも見た目の文字列に近づく方向の変化なので、禁止語の `De&shy;lete` もむしろ当たるようになる（安全側）。表示用の `elementLabel()` は変えない
      ...
      - **ブロック解除語**（`isUnblockWord()`）= `/unblock|ブロック(を)?(すべて|全て)?解除/i`。Cookie 設定のせいで動画・地図などを止めている埋め込みのプレースホルダ（Borlabs の `_brlbs-content-blocker`、Cookiebot の `cookieconsent-optout-*` 等）のボタンで、押すとその埋め込み元への同意になるので、**`scoreCandidates()` はどちらのモードでもこれを候補にしない**（閉じる語の経路も含む）。設定パネル層（§5-5-e）の拒否ボタン判定（`isPanelReject()`）でも同じく除く。拒否語の否定後読み `(?<!un…)` と正規化の組み合わせだけに頼ると、語彙の組み合わせですり抜ける経路が残るため（Borlabs 3 の既定文言 "Accept required service and unblock content" は必要最小系の `(accept|allow|use|enable)…required` に、「ブロックをすべて解除」は拒否語の `(すべて|全て)解除` に当たる）。`decisionCandidates()` には入れない（プレースホルダが容器として採用されること自体は変えず、容器の選び方（§5-5-d の容器検出）で本物のバナーへ進む）
      - **拒否語（reject）**: 強一致（含んでいれば可）= `拒否|拒絶|お断り|同意しない|許可しない|承諾しない|承認しない|受け入れない|利用しない|使用しない|オフにする|無効にする|同意せずに(続ける|進む)|(すべて|全て)解除|選択を解除|オプトアウト|辞退|reject|decline|deny|refuse|disagree|opt out|do not accept|dont accept|do not agree|dont agree|do not allow|dont allow|do not consent|dont consent|withdraw consent|object to|disable(?!d)|turn off|turn all off|continue without|without accepting|(?<!un|dont|donot|ad)block (all )?(cookies|tracking|non)|(?<!un|dont|donot|ad)blockall$`（`オフにする` `無効にする` `利用しない` `使用しない` は日本語のトグル型の拒否ボタン、`turn all off` は "Turn all off"（`turn off` は空白を挟まない並びしか見ないので別に要る）、`without accepting` は "Browse without accepting" 向け。`reject non-essential` `decline all` `deny all` `refuse all` `opt out of all` はすでに裸の `reject` `decline` `deny` `refuse` `opt out` に当たるので足していない。**`解除` は SOFT 禁止語なので、「すべて解除」はヒューリスティックでは押さず、ユーザーが教えたボタンとしてだけ使える**。アポストロフィは正規化で落ちるので `don't` は `dont` で受ける。`do not consent` `withdraw consent` は Google Funding Choices の拒否ボタン文言そのもの、`object to` は IAB TCF の「Object to legitimate interest」に当たる。これらが無いと許可語の裸の `consent` に部分一致して許可語に化けてしまう。`disable` `turn off` はトグル型の拒否ボタン——"Disable non-essential cookies" "Turn off all"——向け。`disable` は過去形を除く `disable(?!d)` にする。"I've disabled my ad blocker" のような「もう切ってある」文言を拒否ボタンにしないため。`block` は目的語が Cookie・トラッキングのもの（"Block cookies" "Block all cookies" "Block tracking" "Block non-essential cookies"）と、正規化後の文言全体が `blockall` のもの（"Block all"。`$` は全文の末尾）だけを拾い、裸の `block` や "Block all …" の前方一致は入れない（"Block all notifications" "Block all messages from this user" よけ）。直前が `un` `dont` `donot` `ad` のものは除く——`un` は Cookie 設定のせいで表示を止めている埋め込みのプレースホルダ「This video is blocked because of your cookie settings.［Unblock video］［Unblock all］」（押すと同意になる）、`dont` `donot` は押すと許可になる "Don't block cookies"（`isAmbiguousReject` は許可の specific 一致しか見ないので防げない）、`ad` は広告ブロッカーの "Adblock all"。"Blocked content" "Block ads" も当たらない。ソフトハイフン（U+00AD）やゼロ幅スペース（U+200B）を挟んだ "Un〈U+00AD〉block all" も、`normalize()` がこれらを除去してから照合するので `(?<!un)` で除かれる）; 必要最小系（**語順を問わない**。"Necessary cookies only" も "Only allow essential cookies" も同じ意味）= `必要なもののみ|必要な.*のみ|必要最小限|必須.*のみ|必要不可欠.*のみ|最低限.*(のみ|だけ)|基本的な.*のみ|necessary.*only|only.*necessary|essential.*only|only.*essential|required.*only|only.*required|strictly necessary(?!.*(optional|analytic|marketing|advertis|performance|targeting|statistic|preference|all|everything|non))|minimum (only|cookies)(?!.*(optional|analytic|marketing|advertis|performance|targeting|statistic|preference|all|everything|non))|minimal (only|cookies)(?!.*(optional|analytic|marketing|advertis|performance|targeting|statistic|preference|all|everything|non))|(accept|allow|use|enable) ?(the )?(strictly )?(necessary|essential|required)(?!.*(optional|analytic|marketing|advertis|performance|targeting|statistic|preference|all|everything|non))|(continue|proceed) with ?(the )?(strictly )?(necessary|essential|required|minimal) ?(cookies?|only)(?!.*(optional|analytic|marketing|advertis|performance|targeting|statistic|preference|all|everything|non))|functional.*only|only.*functional|technical.*only|mandatory.*only|only.*mandatory`（`(accept|allow|use|enable)…(necessary|essential|required)` は `only` の付かない "Accept necessary cookies" 型向け。`accept` の直後が `all` の "Accept all necessary and optional" や、optional・marketing など別カテゴリを含む "Allow necessary and optional cookies" には当たらない＝押すと全許可になる文言を必要最小系から外す。`必要な項目のみ` `必須のみ` `only the strictly necessary` `necessary cookies only` はすでに `必要な.*のみ` `必須.*のみ` `only.*necessary` `necessary.*only` に当たるので足していない。`continue with` `proceed with` は "Continue with necessary cookies" "Proceed with necessary cookies" 型向けで、**目的語（`cookies` / `only`）を必須にしてある**——必要最小系は Cookie 固有語ゲートの外でも有効で SOFT 禁止語・非決定語より優先されるので、目的語を縛らないと "Continue with Essential plan" "Continue with required fields" のようなプラン選択・フォームの「続行」まで押してしまう。`minimal` も同じ理由で `minimal cookies` / `minimal only` と目的語必須の continue / proceed の形にだけ入れ、`accept|allow|use|enable` の名詞には入れない（"Use minimal mode" "Enable minimal UI" "Allow minimal" よけ）。"Accept minimal cookies" は `minimal cookies` 側で拾う（従来は許可語として押されていたが、必要最小系になったので許可語にもならない）。`strictly necessary` `minimum …` `minimal …` `(accept|allow|use|enable) …` `(continue|proceed) with …` の各形には同じ否定先読みを付けてあり、"Accept strictly necessary and marketing" "Accept minimal cookies and all analytics" "Continue with all cookies" のような別カテゴリも含む文言には当たらない）; 弱一致（完全一致のみ）= `no thanks|no, thanks|no thank you|not now|later|maybe later|skip`。弱一致は "No thanks" "Not now" がニュースレター・通知の許可ダイアログにも頻出するため、許可の弱一致と対称に **Cookie 固有語ゲートの内側**でだけ候補にする。**裸の `no` は入れない**（許可の弱一致から `yes` を外したことと対称。酒類・製薬サイトの年齢確認ゲート「Are you over 18? [Yes] [No]」で "No" を押すとサイトから追い出される）
      - **許可語（accept）**: 強一致のうち **容器の文脈なしで押してよい具体的な言い回し** = `(すべて|全て)(の(cookie|クッキー))?(に|を)?(同意|許可|受け入れる)|全部(に)?(同意|許可)|accept all|allow all|enable all|agree (to )?(all|everything)|accept everything|allow everything|yes to all|enable cookies|accept (all )?cookies|allow (all )?cookies`（Cookie バナー以外の同意ダイアログは対象が 1 つなので「すべて」とは言わない。cookie を名指しする "Accept cookies" "Allow cookies" もここに入れる——`isAcceptStrongSpecific` は文言に cookie があれば元から真だが、`isAmbiguousReject` はこの正規表現だけを見るため、連結テキストで拒否語に化けた許可ボタンを reject の候補から外すのに要る。`すべてを受け入れる` `i accept all` は先頭の言い回し・`accept all` に当たるので足していない）、**Cookie 固有語ゲートの側** = `同意する|同意します|承諾する|承諾|許可する|受け入れる|agree|i agree|accept|allow|consent`（`enable` は裸では入れない。"Enable notifications" のような Cookie と無関係なトグルまで拾ってしまうため。"Enable cookies" "Enable all" は具体的な言い回し側の `enable cookies` `enable all` で拾う）; 弱一致（完全一致のみ）= `同意|許可|ok|okay|はい|got it|understood|i understand|continue|sure|fine|alright|thats fine|sounds good|thats ok|thats okay|im ok with that|im okay with that|yes im happy|im happy with that|ok thanks|okay thanks|thanks|thank you|noted|ok i understand|i got it|continue browsing`。`continue` は「次へ」の意味でフォームにも頻出するので一度は外していたが、①容器の Cookie 固有語ゲート ②テキスト入力を内包する容器の除外 の 2 つで守られているため、Cookie 語のある容器の中に限って戻している（"By continuing to browse, you accept our use of cookies. [Continue]" 型の英語バナー用）。**裸の `yes` は入れない**（年齢確認ゲートの "Yes" を押すのは法的な自己申告の代行になる。"Yes to all" は具体的な言い回し側で拾う）。`agree and continue` も入れない（`isAcceptStrong` が先に当たるので採点では到達しない）。`sounds good` `thats ok` `im ok with that` `yes im happy` `im happy with that` などは**同意寄りの相づち**なので、ここにだけ入れて閉じる語には入れない（閉じる語として押す対象にはせず、許可ボタンとして認識させるだけ＝拒否ボタンが無いときの非表示の判定材料になる）。`yes im happy` は theguardian.com の許可ボタン "Yes, I'm happy" の実文言で、完全一致なので裸の `yes` とは別物。`ok thanks` `thanks` `noted` `continue browsing` などの**読んだことを伝えるだけ**の語は、`ok` `got it` `continue` と同じく閉じる語にも入れてある。なお**拒否語（強一致・必要最小系）に当たる文言は許可語として扱わない**ので、"Accept only essential cookies" を accept モードで押すことはない
      - **通知のみバナーの閉じる語**（完全一致）= `ok|okay|閉じる|了解|わかりました|内容を理解した|理解した|了承しました|確認しました|got it|understood|i understand|close|dismiss|continue|ok got it|okay got it|hide|hide this message|hide notice|hide notification|close this notice|no problem|fine|sure|alright|ok thanks|okay thanks|thanks|thank you|noted|ok i understand|i got it|continue browsing|close banner|close message|close notice|close cookie banner|close cookie notice|hide cookie message|×|✕|x`（"OK, got it" は正規化でカンマが落ちて `okgotit` になる）。`close banner` 〜 `hide cookie message` は閉じる操作そのものの語で、同意を意味しないので許可の弱一致には入れない（`hide cookie message` は GOV.UK Frontend の Cookie バナーが選択後に出す確認メッセージの閉じるボタンの実文言。ただし拒否後の「You've rejected additional cookies.［Hide cookie message］」は完了状態の文言に当たって容器にならないので押されず、実際に押されるのは許可後の「You've accepted additional cookies.［Hide cookie message］」だけ）。同意寄りの相づち（"Yes, I'm happy" 等）は入れない。`hide cookie banner` は `hide banner` と同じく**バナーを隠す語**なので折りたたみ語（`COLLAPSE_EXACT`）に入れ、ここには入れない（メッセージを隠す `hide cookie message` `hide this message` だけが閉じる語）。`内容を理解した` `理解した` `了承しました` `確認しました` は、拒否ボタンも設定ボタンも持たない日本語の通知型ダイアログ（dailymotion.com の「内容を理解した」だけの Cookie ダイアログ）の唯一のボタン。`わかりました` `understood` と同じ「読んだことを伝えるだけ」の語で、**完全一致かつ Cookie 固有語ゲートの内側**でしか使わない（「内容を理解して同意する」のような決定を伴う言い回しには当たらない）
...
- `en-continue-with-necessary.html` "We use cookies…" + [Accept all] [Continue with necessary cookies] → reject で "Continue with necessary cookies"（必要最小系）、accept で "Accept all"
- `en-block-all.html` [Allow all] [Block all cookies] → reject で "Block all cookies"（拒否の強一致）、accept で "Allow all"
- `en-thanks-notice.html` 選択肢のない通知 "This site uses cookies…" + [Thanks] のみ → 両モードで閉じる語として押される
- `en-unblock-placeholder.html` 本物のバナー [Accept all] [Reject all] と、同じページ内の埋め込みプレースホルダ（`class="cookie-content-blocker"`。Borlabs の `_brlbs-content-blocker` のような実在の CMP と同じく Cookie を名乗るので**容器として採用される**）「This video is blocked because of your cookie settings.［Unblock video］［Unblock all］」→ "Unblock video" "Unblock all"（押すと同意になる）は両モードで押さず、プレースホルダも消さない。実ブラウザと同じくプレースホルダの方が面積が大きい（テストでは大きさを注入する）と、reject で "Reject all"、accept で "Accept all" が押される。同じ大きさだと DOM 順で先のプレースホルダが最初の容器になるが、押せる候補も hide の見込みも無い容器として飛ばし（§5-5-d の容器検出）、同じく reject で "Reject all"、accept で "Accept all" が押される。プレースホルダは消さず、cloak も残さない。Unblock にソフトハイフン・ゼロ幅スペースを挟んだ派生（`Un&shy;block all` `Un&#8203;block video`）でも Unblock は押さない
- `en-guardian-like.html` theguardian.com 型の [Yes, I'm happy] [Manage or reject cookies] → reject では "Yes, I'm happy"（同意寄りの相づち＝閉じる語ではない）を押さず、設定パネル層が "Manage or reject cookies" を開こうとしてパネルが出ないので非表示に落ちる。accept では "Yes, I'm happy"
...
- `normalize`: 全角→半角、空白・句読点除去、小文字化。約物の除去は英語ボタン向けに 1 種類ずつ検証する（"Don’t accept"→`dontaccept` / "opt-out"→`optout` / "Accept & Close"→`acceptclose` / "Reject all (recommended)"→`rejectallrecommended` / "Why?"→`why`）。`×` `✕` は残ること。不可視の書式文字（U+00AD / U+200B / U+200C / U+200D / U+2060 / U+200E / U+200F / U+2066 / U+034F / U+FEFF）が除去され、`Un&shy;block all` が拒否の強一致にならず、`Re&shy;ject all` は拒否の強一致・`De&shy;lete` は禁止語のままであること
...
- **英語ボタン文言の分類表**: 実サイトでよく見る英語ボタン 60 件以上を `forbidden-hard / forbidden-soft / non-decision / reject-strong / reject-minimal / reject-weak / accept-specific / accept-bare / accept-weak / close` に分類する表を持ち、1 件ずつ検証する。語彙を広げたときに「どのゲートで守られているか」が崩れていないことを見るのが目的なので、期待値は候補の並び順ではなく判定関数の組み合わせそのもの。これとは別に、実在する英語バナーの文言を `scoreCandidates()` に 1 ボタンずつ通す表（`test/en-labels.test.ts`。拒否／許可／閉じる／設定・情報の 4 群）を持ち、採点の結果そのもの（reject で `reject-*`、許可は reject で何も選ばれず accept で `accept-*`、閉じる語は reject で `close`、設定・情報は両モードで何も選ばれない）を 1 件ずつ固定する。"Unblock" 系・"Continue with all cookies" を拒否にしないこと、同意寄りの相づちを閉じる語として押さないこと、年齢確認ゲートの容器では追加した弱一致・閉じる語がすべて無効になることもここで見る
...
- 英語 fixtures の統合: `custom-en` `notice-en` `en-necessary-only` `en-continue` `en-div-buttons` `en-static-banner` `en-continue-with-necessary` `en-block-all` `en-thanks-notice` `en-unblock-placeholder` `en-guardian-like` は期待どおりのボタンが押され（`en-unblock-placeholder` の "Unblock video" "Unblock all"・`en-guardian-like` の reject での "Yes, I'm happy" は押されない。`en-unblock-placeholder` はプレースホルダが容器として採用されることも確かめ、REJECT_STRONG の `(?<!un…)` を外すと落ちる）、`trap-newsletter-en` `trap-tos-en` は両モードで何も押されず消されないこと
...
- ブロック解除語（§5-5-d）: "Accept required service and unblock content" "Always unblock" 「ブロックをすべて解除」だけを持つプレースホルダでは、どちらのモードでも何も押さず消さないこと（プレースホルダが容器として採用されることも確かめる）
- 容器の 2 段の選び方（§5-5-d の容器検出）: 先頭の容器が押せる候補も hide の見込みも無ければ次の容器の拒否 / 許可を押すこと／すべてそうなら従来どおり先頭の容器で `unhandled`（`no-reject`）になり何も押さず消さないこと／先頭が［同意する］だけ（`canHide` のみ真）で後ろに拒否のある別の容器があれば後ろの拒否を押し、先頭は hide しないこと／どの容器にも拒否候補が無ければ hide できる先頭の容器を従来どおり hide すること／fallback=leave でも［同意する］＋設定だけの容器を飛ばさないこと／すべて拒否（`pressCloseOnNotice: false`）で［OK］だけの通知が先頭なら従来どおり hide すること／Cookiebot 型のプレースホルダ（`cookieconsent-optout-marketing`）が同じ大きさで先頭でも本物のバナーの "Reject all" を押し、プレースホルダは hide も cloak もしないこと／プレースホルダが先頭で本物のバナーを内包する即決表の容器があれば、その根を先頭に据えること（進行ログも根まで出す）／猶予中に本物のバナーが出て採用が移ったらプレースホルダの cloak が外れること／内包関係の切り替え（外側 → 内側）では前の容器の cloak を外さないこと
